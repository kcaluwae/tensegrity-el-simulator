...
class Simulator(object):
	def __init__(self,nodes,node_velocities,constrained_nodes,bar_connections,bar_mass,bar_sigma,spring_connections,spring_k,spring_d,spring_l0,nodal_damping=0,two_way_spring=False, external_nodal_forces=None):
		...
		self.ground = np.zeros(shape=(16,1)) # to detect if nodes are on the ground
	...
	def initialize_ode(self,integrator="vode",control_dt = 1000,internal_dt=100,**kwargs):
		'''
	...
	def compute_state(self,state):
		...
		for j in range(self.nodes_eucl.shape[0]):
			if (self.nodes_eucl[j][2] <= -2):
				self.nodes_eucl[j][2] = -2
				self.ground[j] = 1
			else:
				self.ground[j] = 0
